...
extension SwordRPC {
    ...
    func registerUrl() {
        ...
        self._registerUrlMacOS()
        #elseif os(Linux)
        self._registerUrlLinux()
        #else
        print("[SwordRPC] Current platform is not supported. Cannot register URL.")
        #endif
    }
    ...
    #if os(macOS) || targetEnvironment(macCatalyst)
    private func _registerUrlMacOS() {
    ...
    #if os(Linux)
    private func _registerUrlLinux() {
        var execPath = ""
